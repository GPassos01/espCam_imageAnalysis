...
void print_chip_info(void) {
    ...
    // Revisão do chip
    ESP_LOGI(TAG, "📦 Revisão: %d", chip_info.revision);
    
    // Número de cores
    ESP_LOGI(TAG, "⚙️  Cores: %d", chip_info.cores);
    
    // Frequência da CPU
    rtc_cpu_freq_config_t freq_config;
    rtc_clk_cpu_freq_get_config(&freq_config);
    ESP_LOGI(TAG, "🚀 Frequência CPU: %d MHz", freq_config.freq_mhz);
    ...
    ESP_LOGI(TAG, "   Flash: %d MB", flash_size / (1024 * 1024));
    ESP_LOGI(TAG, "   PSRAM: %s", 
             esp_psram_is_initialized() ? "Inicializado" : "Não disponível");
    
    if (esp_psram_is_initialized()) {
        ESP_LOGI(TAG, "   PSRAM Tamanho: %d KB", esp_psram_get_size() / 1024);
    }
    
    // Heap
    ESP_LOGI(TAG, "   Heap Total: ~%d KB", esp_get_free_heap_size() / 1024 + 100); // Estimativa
    ESP_LOGI(TAG, "   Heap Livre: %d KB", esp_get_free_heap_size() / 1024);
    ESP_LOGI(TAG, "   Heap Mínimo: %d KB", esp_get_minimum_free_heap_size() / 1024);
