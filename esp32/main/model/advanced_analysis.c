...
esp_err_t advanced_analysis_init(void) {
    ...
    ESP_LOGI(TAG, "💾 Memória necessária: %zu KB (HVGA otimizada)", required_memory / 1024);
...
void print_memory_efficiency_report(void) {
    ...
    ESP_LOGI(TAG, "💾 PSRAM Total: %zu KB (4MB utilizáveis dos 8MB físicos)", stats.total_psram_kb);
    ESP_LOGI(TAG, "💾 PSRAM Livre: %zu KB", stats.free_psram_kb);
    ESP_LOGI(TAG, "💾 Usado pela Análise: %zu KB", stats.used_by_analysis_kb);
